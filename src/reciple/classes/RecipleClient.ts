...
export class RecipleClient<Ready extends boolean = boolean> extends Client<Ready> {
    ...
    readonly modules: ModuleManager = new ModuleManager({
        client: this,
    });
    readonly cwd: string;
    ...
    protected async _haltCommand(command: AnyCommandBuilder, haltData: AnyCommandHaltData): Promise<boolean> {
        try {
            const haltResolved =
                (command.halt
                    ? await Promise.resolve(command.type == CommandType.SlashCommand
                        ? command.halt(haltData as SlashCommandHaltData)
                        : command.halt(haltData as MessageCommandHaltData)).catch(err => {
                            throw err;
                        })
                    : false) || false;
