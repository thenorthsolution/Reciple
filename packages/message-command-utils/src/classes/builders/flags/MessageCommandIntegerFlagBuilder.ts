...
export interface MessageCommandIntegerFlagBuilderData extends MessageCommandFlagBuilderData<number> {
    max_value?: number;
    min_value?: number;
}

export class MessageCommandIntegerFlagBuilder extends BaseMessageCommandFlagBuilder<number> implements MessageCommandIntegerFlagBuilderData {
    public max_value?: number;
    public min_value?: number;

    constructor(data?: MessageCommandIntegerFlagBuilderData) {
        super(data);
    ...
    public readonly validate = (options: MessageCommandFlagBuilderResolveValueOptions<number>): boolean|Error => {
        for (const value of options.values.map(Number)) {
            if (!value || !Number.isFinite(value) || !Number.isInteger(value)) return new Error(`A value for flag ${options.flag.name} is not an integer`);
