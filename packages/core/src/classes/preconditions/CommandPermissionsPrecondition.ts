...
export class CommandPermissionsPrecondition extends CommandPrecondition {
    ...
    private static async _execute(data: MessageCommandExecuteData, precondition: CommandPermissionsPrecondition): Promise<boolean|CommandPreconditionTriggerData> {
        ...
        const triggerData: CommandPreconditionTriggerData<MessageCommandExecuteData, { type: CommandPermissionsPreconditionTriggerDataType; }> = {
            executeData: data,
            precondition,
            successful: false
        };
        ...
        if (data.message.inGuild() && (requiredBotPermissions || requiredMemberPermissions)) {
            if (requiredBotPermissions && !CommandPermissionsPrecondition.userHasPermissionsIn(data.message.channel, requiredBotPermissions)) {
                triggerData.message = `Bot doesn't have enough permissions to execute this command in this channel`;
                triggerData.data = { type: CommandPermissionsPreconditionTriggerDataType.ClientNotEnoughPermissions };
            ...
            if (requiredMemberPermissions) {
                ...
                if (!member.permissionsIn(data.message.channel).has(requiredMemberPermissions)) {
                    ...
                    triggerData.data = { type: CommandPermissionsPreconditionTriggerDataType.MemberNotEnoughPermissions };
